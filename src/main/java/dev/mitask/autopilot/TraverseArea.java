...
import dev.mitask.AutomaticInfiniteElytra;
...
public class TraverseArea {
    ...
    private static void next(){
        ...
        if(current >= coordinates.size()){
            stop();
            Minecraft.getInstance().player.displayClientMessage(Component.literal("Done"), false);
            return;
        }
        Autopilot.unsetLocation();
        Autopilot.initNewFlight(AutomaticInfiniteElytra.blockPos(coordinates.get(current)), true);
        Minecraft.getInstance().player.displayClientMessage(Component.literal(String.valueOf(coordinates.get(current))), true);
    }
